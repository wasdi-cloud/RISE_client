...
export class ConfirmAdminRegistrationComponent implements OnInit {
  ...
  confirmRegistration() {
    if (!FadeoutUtils.utilsIsStrNullOrEmpty(this.m_sConfirmationCode) && !FadeoutUtils.utilsIsStrNullOrEmpty(this.m_sUserId))
      this.m_oAuthService.confirmAdm(this.m_sConfirmationCode, this.m_sUserId).subscribe({
        next: (oResponse) => {

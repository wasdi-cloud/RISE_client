...
export class RiseUtils{
  ...
  handleError(oError) {
    ...
    if (oError.error.errorStringCodes && oError.error.errorStringCodes[0]) {
      console.log(oError.error.errorStringCodes[0])
      let sTranslatedText = this.m_oTranslationService.getTranslation('ERROR_MSG'+oError.error.errorStringCodes[0]);
      console.log(sTranslatedText)
